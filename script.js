...
// Aggressive music autoplay for mobile - try to play immediately
(function() {
    ...
    if (bgMusic) {
        ...
        const attemptPlay = (tryMuted = false) => {
            if (musicPlayed || !bgMusic) return;
            
            // Try muted first if requested (works better on mobile)
            if (tryMuted) {
                bgMusic.muted = true;
            }
            ...
            if (playPromise !== undefined) {
                playPromise
                    ...
                    .catch(error => {
                        if (!tryMuted) {
                            // If normal play failed, try muted version
                            attemptPlay(true);
                        } else {
        ...
        // Try immediately when script loads (try unmuted first, then muted)
        attemptPlay(false);
        
        // Try on various events for maximum compatibility
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', () => attemptPlay(false));
        } else {
            attemptPlay(false);
        }
        
        window.addEventListener('load', () => {
            setTimeout(() => attemptPlay(false), 50);
        });
        
        // Try on first visibility change
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden && !musicPlayed) {
                attemptPlay(false);
            }
        }, { once: true });
    }
...
// musicPlayed variable is already declared above

// Try to play music as early as possible (even before DOM ready)
(function() {
...
window.addEventListener('load', () => {
    ...
    // Final attempt to play music on load
    const bgMusic = document.getElementById('bgMusic');
    if (bgMusic && !musicPlayed) {
        setTimeout(() => {
            bgMusic.play().then(() => {
...
// Try to play music on page show (when user switches back to tab)
